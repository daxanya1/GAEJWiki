...
public class WikiInlineParserTest {
	...
	public void testWikiInlineParser2() {
		String str = "((([[Ç†Ç†Ç†>https://hogehoge.jp]] ''tes&br;t123'' )))~";
		List<WikiObjectInlineI> wikilist = getTestParser().parseInline(str);
		assertEquals(wikilist.size(), 2);
		assertEquals(wikilist.get(0).toDebugString(), "note|c:/(/:cc:/true|Ç†Ç†Ç†||https://hogehoge.jp|/:cc:/ /:cc:/strong|c:/tes/:cc:/br||/:cc:/t123/:c/:cc:/ )/:c");
		assertEquals(wikilist.get(1).toDebugString(), "<br />");
	}
	...
	public void testWikiInlineParser4() {
		String str = "[[ó˚èKÇR]]Ç‡écÇ¡ÇƒÇ¢ÇÈÅH ";
		List<WikiObjectInlineI> wikilist = getTestParser().parseInline(str);
		assertEquals(wikilist.size(), 2);
		assertEquals(wikilist.get(0).toDebugString(), "page|ó˚èKÇR");
		assertEquals(wikilist.get(1).toDebugString(), "Ç‡écÇ¡ÇƒÇ¢ÇÈÅH ");
	}
